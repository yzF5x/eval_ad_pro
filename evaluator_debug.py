...
def _normalize_sink_head_token_filter_mode(mode: str) -> str:
    aliases = {
...
def _normalize_sink_head_token_topk(value) -> int:
    try:
        topk = int(value)
    except (TypeError, ValueError) as exc:
        raise TypeError(f"sink_head_token_topk must be an integer, got: {value}") from exc
    if topk <= 0:
        raise ValueError(f"sink_head_token_topk must be > 0, got: {topk}")
    return topk
...
def _build_eval_variant_tag(
    ...
    return f"sink_first_token_mean_topk_spike_patches_{topk_spike_patches}{sink_filter_tag}_sharethr_{share_thr:g}"
...
def main(args):
    ...
    sink_head_token_filter_mode = _normalize_sink_head_token_filter_mode(
...
if __name__ == "__main__":
    ...
    if cli_args.sink_head_token_filter_mode is not None:
